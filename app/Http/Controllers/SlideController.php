...
class SlideController extends Controller
{
    ...
    public function postThem(Request $request) {
        ...
        [
            ...
            'NoiDung.required'=> 'Bạn chưa nhập nôi dung slide',
        ]);
        $slide = new Slide;
        ...
        if ($request->hasFile('Hinh')) {
            ...
            if($duoi != 'png' && $duoi != 'jpg' && $duoi != 'jpeg'){
                return redirect('admin.slide.them')->with('loi', 'Bạn phải nhập đúng định dạng ảnh');
            }
    ...
    public function postSua(Request $request, $id) {
        ...
        if ($request->hasFile('Hinh')) {
            ...
            $duoi = $file->getClientOriginalExtension();
            if($duoi != 'png' && $duoi != 'jpg' && $duoi != 'jpeg'){
                return redirect('admin.slide.sua'.$id)->with('loi', 'Bạn phải nhập đúng định dạng ảnh');
            }
            $name = $file->getClientOriginalName();
            $Hinh = str_random(8)."_".$name;
            while (file_exists("upload/slide/".$Hinh)) {
